...
void SBomber::Clone_Objects()
{
    vector<DestroyableGroundObject*> obj = FindDestoyableGroundObjects();
    int var;
    var = rand() % obj.size();

    uint16_t Width = obj[var]->GetWidth();

    for (double i = 0; i < 100; i += Width) 
    {
        bool empty = true;
        for (auto j : obj)
        {
            if ((j->isInside(i, i + Width)))
            {
                empty = false;
                break;
            }
        ...
        if (empty) 
        {
            DestroyableGroundObject* new_obj = obj[var]->clone();
            new_obj->set_X(i);
...
//static const char* ppScroll[ScrollHeight] = { "John"};
//
